        ...
        takeData('edit')
            .then((res) => {
                ...
                for (let i = 0; i < boundInfo.w; i++) {
                    ...
                    bound.position.set(pixel * i, (boundInfo.h - 1) * pixel);
                    bounds.addChild(bound);
                }
                for (let i = 1; i < boundInfo.h - 1; i++) {
                    let bound = new PIXI.Sprite(boundTexture);
                    bound.position.set(0, pixel * i);
                    bounds.addChild(bound);
                    bound = new PIXI.Sprite(boundTexture);
                    bound.position.set((boundInfo.w - 1) * pixel, pixel * i);
                    bounds.addChild(bound);
                }
                ...
                function onSceneMove() {
                    ...
                    if (this === arrowDown) {
                        ...
                        if (pixel * (boundInfo.h - 15) + bounds.y === 0) {
                            arrowDown.visible = false;
                        }
                    }
                    if (this === arrowLeft) {
                    ...
                    if (this === arrowRight) {
                        ...
                        if (pixel * (boundInfo.w - 20) + bounds.x === pixel * 3) {
                            arrowRight.visible = false;
                        }
                    }
                    tool.zIndex = 50;
        ...
        return (
            <div className='container'>
                ...
                <div className='container-btn-group'>
                    <Link to="/play" className="container-btn" style={style}>
                        <img src="../image/icon_play.png" alt="" />
                    </Link>
                    <SaveButton items={this.state.items} play={this.setCanPlay} />
                </div>
            </div>
        )
    }
